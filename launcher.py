...
try:                                        # Older Pythons lack this
    ...
    from importlib.util import find_spec    # check anyway module
except ImportError:
    pass
import platform
import webbrowser
...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="MasterBot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Master",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Master in case of issues",
                        action="store_true")
    parser.add_argument("--update-Master",
                        help="Updates Master (git)",
                        action="store_true")
...
def update_Master():
    ...
    if code == 0:
        ...
        print("\nMaster could not update properly. If this is caused by edits "
              "you have made to the code you can try the repair option from "
              "the Maintenance submenu")
#red

def reset_Master(reqs=False, data=False, Cogs=False, git_reset=False):
    if reqs:
        try:
    ...
    if cogs:
        try:
            shutil.rmtree("moduls", onerror=remove_readonly)
            print("'moduls' folder has been wiped.")
        except FileNotFoundError:
            pass
        except Exception as e:
            print("An error occurred when trying to remove the 'moduls' folder: "
                  "{}".format(e))

    if git_reset:
        code = subprocess.call(("git", "reset", "--hard"))
        if code == 0:
            print("Master has been restored to a more stable area.")
        else:
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Master:")
        print("1. Update Master + requirements (recommended)")
        print("2. Update Master")
...
#red
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Master. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "Master.py")
    ...
    print("Master has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Master without going
    ...
    Bot_Start = "{} --start".format(call)
    Bot_restart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    Bot_Start             = ccd + Bot_Start       + pause
    Bot_restart = ccd + Bot_restart + pause

    files = {
        "Bot_Start"             + ext : Bot_Start,
        "Bot_restart" + ext : Bot_restart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Master Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Master has been "
        ...
        print("1. Run Master /w autorestart in case of issues")
        print("2. Run Master")
        print("3. Update")
        ...
        choice = user_choice()
        if choice == "1":
            run_Master(autorestart=True)
        elif choice == "2":
            run_Master(autorestart=False)
        elif choice == "3":
            update_menu()
        elif choice == "4":
            requirements_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Master needs Python 3.5 or superior in order to function. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Master WILL NOT work without the pip module. Please make sure to "
              "install Python without unchecking any option during the setup")
        wait()
        exit(1)
    if args.repair:
        reset_Master(git_reset=True)
    ...
    if INTERACTIVE_MODE:
        ...
        run_Master(autorestart=args.auto_restart)
