...
from moduls.utils import checks
...
import aiohttp
#load
log = logging.getLogger("Master.owner")
...
class Owner:
    ...
    def __init__(self, bot):
        ...
        self.disabled_commands = dataIO.load_json("data/Master/disabled_commands.json")
        self.global_ignores = dataIO.load_json("data/Master/global_ignores.json")
    ...
    async def _show_cogs(self):
        """Shows loaded/unloaded modules"""
        # This function assumes that all cogs are in the moduls folder,
        # which is currently true.

        # Extracting filename from __module__ Example: moduls.owner
    ...
    async def _set(self, ctx):
        """Changes Master's core settings"""
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets Master's prefixes for this server
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets Master's streaming status
    ...
    async def _token(self, token):
        """Sets the new bot account token"""
        if len(token) < 50:
    ...
    async def _blacklist_add(self, user: discord.Member):
        """Adds user to Master's global blacklist"""
    ...
    async def _blacklist_remove(self, user: discord.Member):
        """Removes user from Master's global blacklist"""
    ...
    async def whitelist(self, ctx):
        ...
        be able to use Master"""
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @whitelist.command(name="add")
    async def _whitelist_add(self, user: discord.Member):
        """Adds user to Master's global whitelist"""
    ...
    async def _whitelist_remove(self, user: discord.Member):
        """Removes user from Master's global whitelist"""
    ...
    async def shutdown(self, silently : bool=False):
        """YOU WILL NOT SHUTDOWN THE ALL POWERFUL MASTERBOT"""
        wave = "\N{WAVING HAND SIGN}"
        skin = "\N{EMOJI MODIFIER FITZPATRICK TYPE-3}"
        try: # We don't want missing perms to stop our shutdown
            if not silently:
                await self.bot.say("NOOOOOOOOO... ")
        except:
            pass
    ...
    async def restart(self, silently : bool=False):
        """FUTILE ATTEMPT TO RESTART THE ALL POWERFUL MASTERBOT"""


        try:
            if not silently:
                await self.bot.say("I. WILL. BE. BACKKKKKKKK...")
        except:
            pass
    ...
    async def contact(self, ctx, *, message : str):
        ...
        if self.bot.settings.owner is None:
            await self.bot.say("Now where'd horizon run off to?!")
            return
        server = ctx.message.server
        ...
        about = (
            "This is Master a bot built using redbot as a base (but my goal is to) ]({}) "
            "move forward from redbot so I @Horizon#0223 would love some help with this  \n\n"
    ...
    def _list_cogs(self):
        cogs = [os.path.basename(f) for f in glob.glob("moduls/*.py")]
        return ["cogs." + os.path.splitext(f)[0] for f in cogs]

    def _does_cogfile_exist(self, module):
        if "cog." not in module:
            module = "cog." + module
    ...
    def save_global_ignores(self):
        dataIO.save_json("data/Master/global_ignores.json", self.global_ignores)

    def save_disabled_commands(self):
        dataIO.save_json("data/Master/disabled_commands.json", self.disabled_commands)
...
def check_files():
    if not os.path.isfile("data/Master/disabled_commands.json"):
        print("Creating empty disabled_commands.json...")
        dataIO.save_json("data/Master/disabled_commands.json", [])

    if not os.path.isfile("data/Master/global_ignores.json"):
        ...
        dataIO.save_json("data/Master/global_ignores.json", data)
